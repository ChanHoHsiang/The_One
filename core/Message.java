...
public class Message implements Comparable<Message> {
	...
	/**	This message generated location */
	private final double x , y;
	
    private double []  interest= new double[5];
	public double[] minterest = new double[5];
	
	//public double[][] totalminterest={};
	 ...
	 * @param interest value of the message ([0,0,0,0,1])
	 */
	public Message(DTNHost from, DTNHost to, String id, int size, double[] minterest) {
		this.from = from;
		this.to = to;
		this.id = id;
		this.size = size;
		...
		this.x = from.getLocation().getX();
		this.y = from.getLocation().getY();
		...
		//System.out.println("message__"+this.minterest[0]+"\n");
		setminterest();
	...
	public void setminterest(){
		
		Random ran = new Random();
		for (int i=0;i<5;i++){
			try{
			this.minterest[i]=(ran.nextInt(5))/4.0;
			}catch (Exception e){
				e.printStackTrace();
			}
			
			//System.out.println("Message____"+this.minterest[0]+"\n");
		}
	...
	public void setInterest(double[] minterest) {
		...
		//System.out.println("Message"+this.interest.length);
	//	System.out.println("Message"+minterest[0]);
	...
	*/
	...
	public Message replicate() {
		Message m = new Message(from, to, id, size, minterest);
		m.copyFrom(this);
		return m;
	}
