...
import virtualcontact.externalvirtualcontact;
...
public class SimScenario implements Serializable {
	...
	/*****   增加一個virtualcontact  ******/
	public static final String VCT_S = "virtualcontact";
	...
	public double [][] totalpinterest={
            ...
            */
            ...
            {0.5, 0.25, 0.5, 0.25, 0.5},//p115 (假設點)

                                    			
	};
	
	
	public void VirtualContact(){
	...
	protected SimScenario() {
		...
		/*************************增加  virtualcontact  ***/
		/*或是在這讀取VCT的表?*/
        //s.setNameSpace(externalvirtualcontact.EXTERNALVIRTUALCONTACT_NS);
	...
	public double[] ranpinterest(int id)
	{
		double [] pinte=new double[5];
		
		
		for(int i=0;i<5;i++){
			
			pinte[i]=totalpinterest[id][i];
		}
		
		//System.out.print("print___interest___"+id+" / "+totalpinterest[id][0]+"\n");
		//System.out.print("ID : "+id+" / pinte[i] : "+pinte[0]);
		return pinte;
	}
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						...
						mmProto, mRouterProto, ranpinterest(j));
				**/
				
				//System.out.println("AA"+ranpinterest(j));
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners,	gid, mmNetInterfaces, comBus, 
						mmProto, mRouterProto, ranpinterest(j));
				hosts.add(host);
